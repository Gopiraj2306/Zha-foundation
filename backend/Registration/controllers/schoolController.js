...
const generateSchoolCode = (state, city) => {
  ...
  const suffix = Math.floor(1000 + Math.random() * 9000);
  return `ZHA0${stateCode}0${cityCode}${suffix}`;
};

exports.createSchool = async (req, res) => {
  try {
    const requiredFields = ['name', 'email', 'school_type', 'address', 'city', 'state', 'postal_code', 'phone', 'mobile', 'management', 'education_district', 'contact_person', 'contact_person_mobile', 'contact_person_email'];
    
    for (const field of requiredFields) {
      if (!req.body[field]) return res.status(400).json({ error: `${field} is required.` });
    }
    const school = await School.create({ ...req.body, status: 'Pending', school_code: null });
    res.status(201).json(school);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
exports.approveSchool = async (req, res) => {
  try {
    const school = await School.findByPk(req.params.id);
    if (!school) return res.status(404).json({ error: 'School not found' });
    if (school.status === 'Approved') return res.status(400).json({ error: 'School already approved' });

    const schoolCode = generateSchoolCode(school.state, school.city);
